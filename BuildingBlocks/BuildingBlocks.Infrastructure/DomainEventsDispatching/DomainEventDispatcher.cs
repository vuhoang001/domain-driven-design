    ...
    IDomainNotificationMapper domainNotificationMapper) : IDomainEventDispatcher
{
    public async Task DispatchEventAsync()
    {
        ...
        {
            /*
             * Đoạn này dùng reflection kết hợp với DI (Autofac) để tạo đúng kiểu notification cho từng domain event khi chạy.
             *
             * B1: lấy template generic IDomainEventNotification<> rồi điền type cụ thể của event bằng MakeGenericType(domainEvent.GetType)
             *     Ví dụ: nếu event là OrderCreatedEvent thì tạo ra IDomainEventNotification<OrderCreatedEvent>.
             * B2: Dùng scope.ResolveOptional để nhờ Autofac khởi tạo instance notification tương ứng (ví dụ OrderCreatedNotifidcation).
        ...
        /*
         * Publish tất cả domainEvents qua Mediartor => xử lý ngay.
         */
        foreach (var domainEvent in domainEvents)
        {
